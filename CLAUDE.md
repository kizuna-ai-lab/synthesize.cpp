...
- **Vendored ggml** (`ggml/`): upstream's tree at the SHA in `ggml/UPSTREAM`, verbatim minus `.github/` — **there are no local patches** as of 2026-07-27; `ggml-patches/README.md` records the five groups that once existed and why each went. Never hand-edit `ggml/`: add a patch file to `ggml-patches/`, then regenerate with `scripts/sync-ggml.sh` (it applies patches in order and aborts loudly if one stops applying); re-vendor with `scripts/sync-ggml.sh <ref>`. Adding a patch also re-blocks converting `ggml/` to a submodule, which the empty patch set now makes possible.
