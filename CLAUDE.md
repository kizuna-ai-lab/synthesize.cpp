...
CUDA development uses the committed presets (`cmake --preset dev-linux-x86_64-cuda` for RTX/sm_89, `dev-dgx-spark` for DGX Spark/sm_121a), each with matching `--build` and `ctest` presets. CUDA builds require CUDA Toolkit 13.3 *exactly*. **CUDA F32 matrix multiplies compute at TF32 and there is no build option to change it** — cuBLAS runs in `CUBLAS_TF32_TENSOR_OP_MATH`, and GGML's own `mmf.cu` takes every F32 matmul with ≤16 columns through `mma...tf32` tiles on Ampere-class or newer devices; expect ~1e-3 relative CUDA-versus-CPU deviation rather than FP32's 1e-7 (see `docs/backends.md`). This sentence described a `SYNTH_CUDA_TF32=OFF` strict-FP32 default until 2026-08-17; that option and the GGML patches gating it were removed on 2026-07-26, and `tests/python/test_cmake_presets.py` asserts the name appears in no preset. Release presets pin exact cubin sets and run a no-PTX verifier — never replace them with `native`/`all`. **A `dev-*` preset proves correctness, never timing**: it inherits `RelWithDebInfo`, which measured 2.19× slower than `Release` on ggml-cpu — use `rel-dgx-spark` (or another `Release`-typed tree) for any performance figure. Full details and the manylinux wheel-build commands: `docs/testing.md`.
...
- **ggml is a submodule** (`ggml/`), pinned to a commit of `ggml-org/ggml` since 2026-07-27 — not vendored, and **it cannot carry a local change**. Clone with `--recurse-submodules` or run `git submodule update --init --recursive`; an uninitialized submodule fails the build at `add_subdirectory(ggml)` rather than degrading quietly. Move upstream with `git -C ggml fetch && git -C ggml checkout <sha> && git add ggml`. Never edit `ggml/` in place: it is a checkout of another repository and the edit vanishes at the next submodule update. `ggml-patches/README.md` records the five local patches that once existed, why each went, and what to do if a local change becomes necessary again.
