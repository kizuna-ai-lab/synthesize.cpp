...
- **Vendored ggml** (`ggml/`): upstream's tree at the SHA in `ggml/UPSTREAM` plus the local patches in `ggml-patches/` (packaging installs, a barrier-deadlock fix, F16 transposed conv, tiled im2col, the strict-FP32 TF32 gates — see `ggml-patches/README.md`). Never hand-edit `ggml/`: change a patch file, then regenerate with `scripts/sync-ggml.sh` (it applies patches and aborts loudly if one stops applying); re-vendor with `scripts/sync-ggml.sh <ref>`.
