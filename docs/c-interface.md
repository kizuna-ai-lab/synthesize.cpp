...
The initializer writes the caller-provided structure size, clears all visible fields, and sets `seed` to zero when that field is present. There is still no valid null-parameter default, and the amendment above does not create one: the parameter structure itself is required and a null `params` returns `SYNTH_ERR_INVALID_ARG`, on every variant. An absent parameter structure is not a spelling of an empty description -- it also withholds the language tag, the seed, and the diagnostics sink, which the Interface has no basis for inventing. A `struct_size` that is VALID -- at least `sizeof(uint64_t)`, per the parameter-structure rules above -- but too small to expose `description` is a different case: the visible-field rules already give the missing fields their cleared defaults, so such a call presents an empty description and resolves exactly as an explicitly empty one does on that variant. Below that floor nothing is presented at all: a `struct_size` under `sizeof(uint64_t)`, zero included, is rejected with `SYNTH_ERR_BAD_STRUCT_SIZE` as the parameter-structure rules say, on the supporting and non-supporting arms alike, and never reaches the emptiness question. Measured across the real seam 2026-08-19: sizes 0, 4 and 7 return `SYNTH_ERR_BAD_STRUCT_SIZE`; 8, 12 and 15 return `SYNTH_OK` with the same result as an explicitly empty description.
