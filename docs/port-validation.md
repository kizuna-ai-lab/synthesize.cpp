...
| `cases` | Twelve to thirty-two deterministic Port Validation Cases. **Exemption, added 2026-08-18, closed 2026-08-19:** a manifest may instead declare `suite_status: "incremental"` and commit with as few as zero cases, provided it grows toward the twelve-case floor across the tasks named in its own plan rather than staying there. `qwen3-tts-12hz-1-7b-voicedesign` was the first and, as of Stage 3 Plan 2 Task 7, the only manifest ever to use it: its Golden Manifest was committed at 0 cases in Stage 3 Plan 1's Task 6 and stayed at 0 through Stage 3 Plan 1's own Task 7 (which gated the empty-instruct rung with an integration driver outside the manifest — a `replay`-stage tolerance cell and fault-injection figure, not a case) and through Stage 3 Plan 2's Tasks 1-6. Stage 3 Plan 2's Task 7 is what filled it: thirteen cases landed in one commit and `suite_status` was removed, re-engaging the twelve-case floor for this manifest the same way every other manifest has always met it. The schema enforces this as an `if`/`then`/`else` keyed on `suite_status` (`docs/schemas/synthesize-golden-manifest-v1.schema.json`), not as a lowered floor: omitting the field is the default and still means twelve to thirty-two, exactly as it always has, so a manifest cannot go below the floor by neglecting to set anything. **The allowlist is empty.** `tests/python/test_golden_manifests.py`'s `SUITE_STATUS_ALLOWLIST` names, independently of the schema, exactly which manifests may carry `suite_status` at all — checked in both directions (the carrying set equals the allowlist; a manifest carrying the flag has zero cases, a manifest without it has at least twelve) so the schema's own waiver and this file's two case-count exemptions cannot silently disagree about which manifest the exemption applies to. Before 2026-08-19 the two mechanisms did not reference each other: the schema waived the floor for whoever set the flag, this file's exemptions waived themselves for whoever had zero cases, and a manifest could carry the flag WITH cases or have zero cases WITHOUT it, undetected either way — demonstrated live against a 3-case copy of `vits-ljspeech`'s own committed manifest, which validated clean once `suite_status: "incremental"` was added to it. |
