...
**A Listening Audit cannot gate this plan.** Plan 2 could lean on one: an x-vector that is wrong in its last decimal place still produces speech, but an x-vector that is wrong in *structure* produces the wrong voice, and a listener hears that. ICL is not like that. The research established the property plainly, and this family's own source comments have been warning about it since Stage 1 (`src/arch/qwen3-tts/talker-host.h`, `:110-111` at the branch tip: *"A prompt off by one still synthesizes speech, in the wrong voice or the wrong language"*). A two-track prompt whose alignment is wrong — text padded where it should have been truncated, the codec track offset by one, the sixteen groups summed in the wrong order — still returns `SYNTH_OK` with finite, non-silent audio, and no end-to-end audio tolerance can catch it either, because the talker is sampled and the audio was never going to match sample-for-sample anyway.

> **CORRECTED 2026-08-14, after execution.** This paragraph originally read that such a prompt "produces fluent, plausible, correctly-languaged speech in approximately the reference voice. **No listener can distinguish that** from a correct implementation" — an audible judgement offered as the plan's own justification for not running a Listening Audit, and never tested against a listener. **No ICL Listening Audit has run, before or since.** What Task 11 measured under a one-frame codec rotation is: `SYNTH_OK`, 19,200 finite PCM frames against 24,960 correct, peak amplitude 0.7696, and every end-to-end differential in the suite still passing. Nobody knows whether a listener would have distinguished it. The conclusion the paragraph draws — that the numerical gates are the whole defence — is what the execution actually established, and it stands without the embellishment. The `talker-host.h` citation was also stale twice over (`:79-80` at the base commit) and, read in full, says the opposite of what the surrounding sentence used it for.
...
- **SUPERSEDED — measured at Task 10, this whole bullet is wrong. The accepted tag is `"en"` and the comment at `:250-257` was right; do not "fix" it.** `src/arch/qwen3-tts/model.cpp:178-196` deliberately bridges the package's full names to BCP-47 (`{ "en", "english" }` and nine more, with a comment saying why), `model.cpp:365` publishes the BCP-47 side, and `declared_language` (`src/voice-profile.cpp:176-181`) matches exactly — so `"english"` cannot pass and `"en"` can. Three vocabularies were conflated: the package's raw metadata, the public interface, and the Golden Manifest's description of the *oracle's* vocabulary. Tests on this path assert the accepted tag **from the package**, hardcoding neither spelling. The rule this bullet states — shape first, then declared-vocabulary membership, refused by two *different* statuses — is unaffected and stands. The superseded text follows, kept because the next reader needs to see which reading was wrong: ~~**The accepted tag is `"english"`, not `"en"`, and the existing comment at `:250-257` is wrong about this.**~~ That comment calls `"en"` "a real, package-declared tag". The Base package declares its languages as full English names — `general.languages = ['chinese','english','french','german','italian','japanese','korean','portuguese','russian','spanish']`, and `synthesize.qwen3-tts.languages.names` likewise — and every Golden Manifest case carries `reference.language_tag` values from that set (`"english"`, `"chinese"`, `"japanese"`). Under Step 3's validation `"en"` passes `valid_bcp47_shape` and then fails `declared_language`, so it is refused with `SYNTH_ERR_UNSUPPORTED_LANGUAGE`. Rewrite the function to accept `"english"` and to keep refusing `"en"` and `"zz-ZZ"` — now by their correct, *different* statuses rather than identically — and correct the stale comment in the same change, since a false comment about which tags this package declares is exactly what would send the next reader to `"en"`.
...
Flip the capability back to `UNSUPPORTED` and confirm the rewritten tests fail. Drop the `declared_language` check so an undeclared tag is accepted, and confirm the language test fails — **the pair to use is a well-formed but undeclared tag such as `"zz"`, NOT `"en"`, which is the accepted one** (see the superseded bullet above). Drop the `valid_bcp47_shape` check and confirm the malformed-shape case stops returning `SYNTH_ERR_INVALID_ARG`.

**These two halves do not fail independently, and Task 10's review measured why.** `declared_language` refuses a malformed tag as well, so dropping `valid_bcp47_shape` alone changes only the *status* — `SYNTH_ERR_UNSUPPORTED_LANGUAGE` instead of `SYNTH_ERR_INVALID_ARG`. The status is the load-bearing part and a status-blind assertion would pass either way. This is the same mutual-masking property Task 9 recorded on `prescan_buffer`: single-deletion inversion cannot see it, so name the partner rather than concluding a check is dead.
...
**SUPERSEDED at Task 12 — the premise below was false, it blocked the task, and it is corrected here rather than deleted so the reasoning survives.** "Both sides are `{public, replay}` today" stopped being true inside this very plan: Task 5's fix round committed `scripts/validate-qwen3-tts-codec-encoder.py`, making the validator set `{codec-encoder, public, replay}` and failing `test_every_registered_validator_has_a_measured_stage` for both variants. Nobody saw it for six tasks because **`test_tolerance_coverage` was not registered with CTest** — Task 12 was the first task told to invoke it, and registering it (`synthesize-tolerance-coverage`) is part of that task's delivery. Two further corrections: the mismatch was a *separator* (stage `codec_encoder`, validator stem `codec-encoder`), which the `provisional_variants` note had anticipated by naming the underscore filename as the one that "does not exist yet"; and the exact-set-equality rule is **unsatisfiable, not merely inconvenient**, whenever a capability is variant-specific — CustomVoice has no codec encoder at all (894 − 657 = 76 speaker-encoder + 161 codec-encoder tensors, Base-only). The rule now requires every validator to have a measured stage *somewhere* and every measured stage to have a validator, which is identical for a single-variant family and restores the intent instead of relaxing it. The `codec_encoder` stage was promoted into the Base grid and `provisional_variants` deleted. The superseded text follows:

~~`tests/python/test_tolerance_coverage.py:129-148` asserts **exact set equality** between the validator basenames globbed from `scripts/validate-qwen3-tts-*.py` (stripped of the prefix) and each variant's reference-profile stage set. Both sides are `{public, replay}` today.~~ **A `codec_encoder` stage therefore cannot be promoted into `variants` without a `scripts/validate-qwen3-tts-codec_encoder.py`** — and creating one would force a `codec_encoder` stage into CustomVoice's `BF16`/`F16`/`Q8_MIXED` grids and their CUDA sub-grids, five cells describing a subsystem that variant does not have. Plan 2 hit this exact wall and its Task 6 resolved it by landing the gate at **probe** granularity inside the existing `replay` stage, via a new comparison **mode** on `validate-qwen3-tts-replay.py` rather than a new basename (plan-2 file-structure row `:61`). Do the same.
