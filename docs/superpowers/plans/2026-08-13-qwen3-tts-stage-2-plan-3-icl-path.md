...
Compare per stage against `rvq_residual_s00..s15.f32`, then compare `codes.i32` for **exact equality** against `codes/reference.i32` on all three materialized cases, through test 5's production comparison function. Because the layout settlement above makes the two buffers byte-identical, this comparison reads the file straight into a flat `int32_t` buffer and compares it element for element — **no transpose, no reshape**. If a transpose appears necessary, the port built a stage-major grid and test 5 should already have caught it.

- [ ] **Step 3: Report the tie margin, and let it decide the gate's form**

Read `min_distance_margin` from Task 1's `conventions.json` and record the port's own narrowest gap over the same cases. Per `spec:365-374`: **if no flip occurs, the gate is plain equality**; if flips occur, admissible grids are enumerated through `oracle.alternate_grids` — schema at `docs/schemas/synthesize-golden-manifest-v1.schema.json:625`, rule at `docs/port-validation.md:63`, worked precedent in `tests/golden/omnivoice/omnivoice-0-6b.manifest.json:2303`, `scripts/validate-omnivoice-replay.py:276, 393`, and the committed-digest test `test_alternate_grids_are_committed_and_match_their_digests` (`tests/python/test_golden_manifests.py:312-336`, the sha256 compare at `:331-336`). **The comparison is never relaxed.** This plan does not predict which branch fires; this step measures it and Task 12 commits the result.
...
- The language rejection becomes **validation**, not acceptance. The comment at `:508-514` records that a reviewer measured `language_tag="zz-ZZ"` returning `SYNTH_OK` before the blanket refusal went in, and names OmniVoice's `declared_language` as the shape that would have refused it. A tag that is not one of the package's declared languages is refused with `SYNTH_ERR_UNSUPPORTED_LANGUAGE`; a shape that is not BCP-47 is refused with `SYNTH_ERR_INVALID_ARG`. **Do not replace a blanket refusal with a blanket acceptance** — that would reintroduce the exact defect the refusal closed.
  ...
  **What "declared" means for this package is a full English name.** `general.languages` holds `chinese english french german italian japanese korean portuguese russian spanish`. A caller passing `"en"` is refused with `SYNTH_ERR_UNSUPPORTED_LANGUAGE` — correct behaviour, and a surprise worth stating, because "a BCP-47 language tag" reads as `"en"` to almost everyone. Every test written against this path uses `"english"` for the accepted case.
...
2. **Whether RVQ tie flips occur at all**, and therefore whether the codes gate is plain equality or `alternate_grids` (Task 5 Step 3).
