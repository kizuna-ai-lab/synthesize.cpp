...
**The unexercised branch.** Upstream's alignment rule (`qwen_tts/core/models/modeling_qwen3_tts.py:2015-2019`) has two arms: when `T1 > T2` the text track is truncated to `T2` and its tail becomes the trailing schedule; otherwise the text track is padded up to `T2` with `tts_pad_embed` and the trailing schedule is a bare `tts_pad_embed`. The research reports that every ICL artifact on disk exercises only the second arm. **This plan cannot verify that claim from the artifacts, and neither can the implementer**: the summed block is `T2` positions long in *both* arms, so `prompt/icl_embed.f32`'s size is silent about which one ran. The discriminator is the trailing schedule, which nothing on disk records. Two obligations follow, and neither is optional:

- Task 1 dumps `T1`, `T2`, the branch taken, and the trailing schedule as first-class artifacts, so the branch stops being an inference.
- Task 12 requires at least one measured case in **each** arm. `base-ref-min` (1 s reference, 13 reference frames, `T2 = 14`, against a reference transcript far longer than fourteen tokens) is the strongest candidate for the truncating arm and `base-icl-en` (101 reference frames, `T2 = 102`) for the padding arm — but which arm each actually takes is Task 1's measurement to report, not this plan's to assert.

---

## Global Constraints

- Spec: `docs/superpowers/specs/2026-08-11-qwen3-tts-stage-2-design.md`, sections 3–7 and 9, **including its three errata**. Carry-over ledgers: `docs/superpowers/plans/2026-08-13-qwen3-tts-stage-2-plan-2-carryover.md` and `docs/superpowers/plans/2026-08-12-qwen3-tts-stage-2-plan-1-carryover.md`. Structural template: `docs/superpowers/plans/2026-08-12-qwen3-tts-stage-2-plan-2-xvector-path.md`.
...
- Base package facts that must hold: **894 tensors**, of which the codec encoder is **161 emitted** — `1 + 28 + 96 + 3 + 33`, arithmetic at `catalog.cpp:621-625`. The design's §1.1 row saying 225 is the *raw safetensors* count (225 raw − 32 collapsed EMA-accumulator pairs − 32 `.initialized` flags = 161, `docs/porting/families/qwen3-tts.md:1987-1999`). **A graph is built from the emitted set, so 161 is the number that matters here.** `expected_tensor_count` must not move.
...
| `src/arch/qwen3-tts/codec-encoder-host.h` / `.cpp` | create | Frame arithmetic against 1920, the host-side RVQ argmin, the 16-of-32 slice, code range checks, tie-margin instrumentation |
...
| `tests/golden/qwen3-tts/qwen3-tts-12hz-0-6b-base.manifest.json` | modify | Materialized ICL cases; `alternate_grids` only if ties are measured |
...
The package publishes **no** `codec.encoder.*` metadata namespace at all (research §1e); the catalog's encoder widths are compiled-in literals at `catalog.cpp:429-434`. Read `models/qwen3-tts-12hz-0-6b-base/speech_tokenizer/config.json` and write its `encoder_config.*` into `conventions.json` beside the upstream lines. The reconciliation that must hold, checked here rather than discovered in Task 3:

| Quantity | Checkpoint `encoder_config` | What the port derives | Source of the port's value |
| --- | --- | --- | --- |
| transformer hidden | `hidden_size` 512 | `codec.decoder.codebook_dim` = **512** | GGUF metadata `synthesize.qwen3-tts.codec.decoder.codebook_dim` |
| transformer MLP | `intermediate_size` 2048 | `4 * hidden` = 2048 | `catalog.cpp:557` |
| transformer layers | `num_hidden_layers` 8 | `kCodecEncoderTransformerLayerCount` = 8 | `catalog.cpp:433` |
| heads / head_dim | 8 / 64 | 8 × 64 = 512 = hidden | `encoder_config` |
| RVQ projected width | `codebook_dim` 256, `vector_quantization_hidden_dimension` 256 | `codebook_dim / 2` = **256** | `catalog.cpp:409` |
| codebook | `codebook_size` 2048 | `{codebook_dim/2, codebook_size}` = `{256, 2048}` | `catalog.cpp:412` |
| stage widths / kernels | `num_filters` 64, `compress` 2 | `{128, 256, 512, 1024}` / `{8, 10, 12, 16}` | `catalog.cpp:430-431` |
| frame downsampler | — | kernel 4, 512→512, **no bias** | `catalog.cpp:432, 546-547` |
| samples per frame | `encode_downsample_rate` 1920 | `synthesize.qwen3-tts.codec.hop_length` = 1920 | GGUF metadata; `scripts/convert-qwen3-tts.py:131-132` |
| quantizers read | `encoder_valid_num_quantizers` 16 | `quantizer_count` 16 = 1 semantic + 15 acoustic | GGUF metadata |

Two things this table settles and one it does not. The encoder RVQ carries **31** acoustic stages (`kCodecEncoderAcousticQuantizerCount` at `catalog.cpp:434`) of which only 15 are ever read, so sixteen catalogued codebooks are resolved and never evaluated — record that as intended, not as a bug. The SEANet strides 4·5·6·8 = 960 times the downsampler's stride 2 gives 1920 samples per frame, which the artifacts already confirm (193,920 → 101 frames; 24,000 → 13; 720,000 → 375; the trim is a **ceiling** divide — 24,000/1920 = 12.5 → 13). What it does **not** settle is whether the strides are 4/5/6/8 in that order; read that off `modeling_mimi.py` and record it, because a reversed stride order produces the same total and a different everything else.
...
The RVQ needs more than a hook. For each of the 16 read stages, dump the residual *entering* the stage and, per frame, the two smallest squared distances over the 2048 codebook entries. The second is what makes the design's §6 tie question answerable:

```python
# The tie margin is the whole reason this dump exists in this shape. RVQ
# assignment is an argmin over codebook distances; two implementations that
# differ only in F32 accumulation order flip an assignment exactly when the
# gap between the best and second-best entry is smaller than that difference.
# docs/port-validation.md handles flips with oracle.alternate_grids, but the
# spec (section 6) requires MEASURING whether they occur before choosing a
# gate -- so the margin is an artifact, not a diagnostic print.
...
git commit -m "qwen3-tts: dump every codec-encoder stage and its RVQ tie margins, not just the codes"
```
...
- Produces: under `<case-id>/prompt/` beside the existing `icl_embed.f32` — `text_track.f32`, `codec_track.f32`, `trailing.f32`, `ref_text_ids.i32`, `target_text_ids.i32`, and an `alignment.json` carrying `T1`, `T2`, `branch` (`"truncate"` when `T1 > T2`, `"pad"` otherwise), `ref_frames`, and `trailing_positions`. Tasks 6, 7 and 12 compare against these.
...
From `qwen_tts/core/models/modeling_qwen3_tts.py:1968-2019` (`generate_icl_prompt`), called at `:2189-2196`. Into `conventions.json`:
...
**Then report the branch for each case, and do not assume it.** The block is `T2` positions long in both arms, so its size proves nothing. `base-ref-min` has `T2 = 14` against a reference transcript of "Okay. Yeah. I resent you. I love you. I…" — visibly more than fourteen tokens — so `T1 > T2` and the **truncating** arm is the expected report. If all three cases report the same branch, Task 12 must construct a case for the other one. Write the observed branch of each case into `alignment.json`; that file is what Task 7's tests are written from.
...
Then change `resolve_codec_encoder`'s signature to take a `CodecEncoderWeights &` target, delete `Conv1dWeights scratch;` and `LayerNormWeights norm_scratch;`, and update the single call site at `catalog.cpp:641-644`. Rewrite the header comment at `:510-515` — it is now false, and leaving it would tell the next reader that Plan 3 has not run.
...
5. **The layout is pinned.** The design's D5 table writes reference codes as `[16, T]`; the oracle's `codes/reference.i32` is asserted `[frames, 16]` (`dump_reference_qwen3_tts_base.py:420-426`). Pick one for `CodecEncoding`, state it in the header, and write a test that a transposed grid is not accepted as equal — a transposed 16×16 grid is the one case where a shape check alone passes.
...
- [ ] **Step 2: Implement, then measure against the oracle**

Compare per stage against `rvq_residual_s00..s15.f32`, then compare `codes.i32` for **exact equality** against `codes/reference.i32` on all three materialized cases.

- [ ] **Step 3: Report the tie margin, and let it decide the gate's form**

Read `min_distance_margin` from Task 1's `conventions.json` and record the port's own narrowest gap over the same cases. Per `spec:365-374`: **if no flip occurs, the gate is plain equality**; if flips occur, admissible grids are enumerated through `oracle.alternate_grids` — schema at `docs/schemas/synthesize-golden-manifest-v1.schema.json:625`, rule at `docs/port-validation.md:63`, worked precedent in `tests/golden/omnivoice/omnivoice-0-6b.manifest.json:2303`, `scripts/validate-omnivoice-replay.py:276, 393`, and the committed-digest test `tests/python/test_golden_manifests.py:312-325`. **The comparison is never relaxed.** This plan does not predict which branch fires; this step measures it and Task 12 commits the result.
...
```bash
git commit -m "qwen3-tts: reference waveform to [16, T] codes, measured against the oracle"
```
...
**Interfaces:**
- Produces: `qwen_reference_turn(const std::string &)` and a `kReferenceSuffixTokens` constant beside the existing pair, plus a `Model::tokenize_reference_transcript` entry point.
...
4. **The boundary case that motivates the whole idiom**: a transcript beginning with a space, and one beginning with a non-ASCII character, tokenize to a different id sequence under the wrapper than bare. If they do not differ for any input tried, say so in the test's comment rather than deleting the test — the claim being pinned is that the port uses the wrapper, not that a difference was observed.
...
3. **The padding arm**: with `T1 <= T2`, the block is `T2` positions, positions `T1..T2-1` of the text track are `tts_pad`, and the trailing schedule is a single `tts_pad`. **If Task 2 reports that no case on disk takes this arm, this test is the only thing exercising it until Task 12 lands a case — say so in the test's comment.**
...
**Interfaces:**
- Consumes: `Model::prepare_codec_reference` (Task 5), `Model::tokenize_reference_transcript` (Task 6), `Model::prepare_x_vector` (Plan 2).
- Produces: `CloneMode::Icl`, an ICL payload, and `create_icl_profile(...)` beside `create_x_vector_profile`.
...
1. **A transcript makes an ICL Profile; its absence makes an x-vector Profile** (D4). Same call, two transcripts, two `CloneMode` values.
2. **An empty or whitespace-only transcript is `SYNTH_ERR_INVALID_ARG`** with a named diagnostic code, per `spec:419` — not "treated as absent". This is the distinction Plan 2's `test_a_whitespace_transcript_is_rejected_too` was written early precisely to preserve.
3. **The ICL payload carries all three**: a `[1024]` x-vector, `[16, T]` codes, and a non-empty id vector.
4. **The codes and the reference audio agree**: a Profile prepared from a 1 s clip carries 13 frames; from the 8.08 s clip, 101. Derived from `hop_length` metadata, not a literal.
...
Delete the mode selection and confirm test 1 fails with both Profiles reporting `XVector`.
...
**The scope trap.** `PrescanKeyScope`'s own header comment (`profile.h:148-159`) says the enum exists because a flat key-set check misses a key **moved between kinds**, or a per-kind count left stale. If the ICL writer emits `ref_rms` and `language_tag` — which it should, since the ICL payload carries both — then those two entries move from `kXVectorOnly` to `kCommon`, and **that move must land with both count constants updated in the same change**. This is exactly the defect the enum was built to catch; letting it fire is the point.
...
5. **An ICL envelope with an x-vector-only key missing is refused.**
...
- `tests/qwen3_tts_base_load_real.cpp:219-283` — `check_reference_transcript_refused` (asserting diagnostic code `voice_profile.transcript_unsupported` at `:243`) and `check_reference_language_refused` (`:280`), both wired at `:547-548`, become **mode-selection** checks: a transcript now yields a Profile whose `kind` is `icl`, and a valid language tag is accepted.
...
- The transcript rejection becomes mode selection: present ⇒ ICL Profile, absent ⇒ x-vector Profile. **An empty or whitespace-only transcript stays `SYNTH_ERR_INVALID_ARG`** (`spec:419`).
...
`create_x_vector_profile`'s own defence-in-depth transcript refusal (`profile.h:91-98`, unreachable today per `voice-profile.cpp:559-570`) becomes the dispatch to `create_icl_profile`; its header comment already says "Plan 3 is the change that turns this rejection into the mode selector".

- [ ] **Step 4: Prove, gate, commit**

Flip the capability back to `UNSUPPORTED` and confirm the rewritten tests fail. Accept `"zz-ZZ"` and confirm the language test fails.
...
The inherited record, and it is inherited **unresolved** (Plan 2 carryover §2.2; Plan 1 carryover §2.2): a 30 s reference (720,000 samples, looped 4×, 375 reference frames) produced **9 generated frames / 0.72 s** for an 11-word sentence that takes ~45 frames at the clip's native length. Regenerating that case *in x-vector mode* gave 46 frames — **which is not evidence about ICL**, and the carryover says so twice. The artifacts survive at `build/qwen3-tts-reference-bounds/dur-30s.report.json`: `generated_code_frames 9`, `duration_seconds 0.72`, `peak_abs 0.4746`, `rms 0.0686`, `distinct_semantic_codes 8`, `most_common_semantic_code_fraction 0.222`, `reference_code_frames 375`, `prompt.icl_embed` shape `[1, 376, 1024]`. That case can be driven again without re-sourcing audio.
...
One structural observation is available and is offered as a **hypothesis, not a finding**: at 375 reference frames `T2 = 376` far exceeds `T1` (reference transcript + a short target text + eos), so upstream takes the padding arm, the entire target text is consumed *inside* the prompt, and the trailing schedule is a bare `tts_pad_embed`. Whether that, or the 4× loop of the same clip, or something else drives the early EOS is what this step adjudicates. Do not write the hypothesis into the family record as a cause.
...
- `codec.reference_codes` — a discrete-equality probe, or the `alternate_grids` form if and only if Task 5 Step 3 measured real flips;
...
Move `variants[base].status` off `"partial-measurement-speaker-path-only"` and `replay.cases` off `2`. Every number is measured in this task; **this plan prescribes none of them.**
...
The commit message states the observed values, the case ids, the backend, the build, which case covers which alignment arm, and the 9-frame adjudication's outcome — the tolerance schema has no field for any of that, so the commit is the only carrier.
...
- Produces: the spec's §8 gate — "ICL clone runs end to end; codes and prompt layout meet oracle tolerances; Golden Manifest covers both modes."
...
1. A Profile prepared with a transcript reports `kind` `icl` and carries codes matching `codes/reference.i32` exactly.
...
**D5's declaration does not exist yet, and it is a confirmed-contract change.** `docs/voice-conditioning.md` currently states that Profiles "omit enrollment audio, transcript, and description source material by default" — and that sentence, unamended, is now incomplete for this family. D5 (`spec:112-121`):
...
State, in the document's own register: a Model Family Profile Schema may declare derived conditioning that is invertible to its source, and where it does, the Schema declares the field explicitly and this document names the consequence — **a Serialized qwen3-tts ICL Profile carries reference text token ids, byte-level BPE is invertible, and whoever holds the Profile can recover the reference transcript**. Enrollment audio is still never carried. Update the document's `Status: Confirmed …` line with today's date, per the documentation convention.

This is a change to a confirmed contract, so it is a change to what the project promises — write it as a promise, not as a note.

- [ ] **Step 2: Extend `docs/porting/families/qwen3-tts.md` with a "Stage 2, Plan 3" section**

Carrying: the codec-encoder conventions from Task 1's `conventions.json` with their `transformers==4.57.3` line numbers, flagged as this family's **third provenance**; the reconciliation table from Task 1 Step 2; the measured codec and prompt tolerances with case ids, backend and build; the two alignment arms and which case covers each; the 9-frame adjudication's outcome; the Profile Schema's two `kind` values with the note that `icl` landed without a version bump; which of the six family-conditioned dispatch points Plan 3 changed; and what remains — Description Text (Stage 3), the CLI, quantization, CUDA, and the Plan 4 listening pass. Correct the record's tensor-count wording so 225 (raw) and 161 (emitted) are never again confusable. Update the `Status:` line.
...
**3. The prompt representation change is not local.** Widening `TalkerInputPosition` from one code to sixteen touches `flatten_talker_prompt`'s contiguous-run invariant, `codec_offset`, `external_speaker_index`, and three call sites in `model.cpp`. **Mitigation:** Task 7's test 8 asserts a Plan 2 x-vector prompt is unchanged, and `build_talker_prefill_input`'s new parameters default so every Stage 1 call site compiles untouched.

**4. Discreteness hides errors until it does not.** The encoder's output is quantized, so a latent that is slightly wrong produces codes that are exactly right — until a clip where a margin is narrow, and then one code flips and the voice changes. **Mitigation:** every intermediate stage is compared as F32 before the codes are compared as integers, and Task 1 dumps the per-stage distance margin so the flip risk is a measured number rather than an assumption.
...
**Spec coverage.** D1's second mode is the whole plan. D4's "mode fixed at preparation" is Task 8 and Task 10. D5's three payload rows are Task 8 and its recoverability declaration is Task 14 — the one D5 obligation that was entirely absent from the tree. D6 is unchanged and re-asserted at Task 11 Step 2. D7's limits are unchanged and explicitly protected in Global Constraints. Section 3's `OPTIONAL` flip is Task 10, with the design's own wording quoted rather than paraphrased and its "statement about the runtime, not the package" half made a constraint. Section 5's two remaining module rows (`codec-encoder{,-host}`) are Tasks 4 and 5. Section 6's two missing dump scripts are Tasks 1 and 2; its discrete-equality rule and its `alternate_grids` escape are Task 5 Step 3 and Task 12 Step 4; its seeded-sampling erratum governs Task 12 Step 1. Section 7 is explicitly refused as Plan 4's. Section 9's error table: the empty-transcript row is Tasks 6 and 8, the duration and clip-count rows are unchanged from Plan 2's Arm A, the input-limit row rides Stage 1's existing path, the compatibility row is Task 9, the no-Profile row is Task 11, the non-finite-PCM row is the Audio Normalizer's.
...
**Unknowns carried as unknowns.** Three, each with the task that establishes it: which alignment arm each existing case takes (Task 2 Step 3); whether RVQ tie flips occur at all, and therefore whether the codes gate is plain equality or `alternate_grids` (Task 5 Step 3); and what causes the 9-frame render, which Task 12 Step 3 adjudicates with three named outcomes and an explicitly-labelled hypothesis that must not be promoted to a cause. None of the three is written anywhere in this plan as a decided fact.
