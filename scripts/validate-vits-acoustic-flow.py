#!/usr/bin/env python3
"""Compare the C++ VITS reverse acoustic flow with pinned PyTorch artifacts."""
...
def main() -> int:
    ...
    for case in manifest.get("cases", []):
        ...
        output_dir = output_root / case_id / "cpp" / (
            "acoustic_flow" if args.backend == "cpu" else f"acoustic_flow_{args.backend}"
        )
        ...
        probe = compare_f32(
            reference_dir / artifact_path(case, "flow.z"),
            output_dir / "z.f32",
        )
        case_reports.append({"id": case_id, "probes": {"flow.z": probe}})

    if not case_reports:
        raise ValidationError("manifest has no cases")
    worst_case = max(case_reports, key=lambda item: item["probes"]["flow.z"]["max_abs"])
    worst = {"flow.z": {"case_id": worst_case["id"], **worst_case["probes"]["flow.z"]}}
    report = {
    ...
    report_name = validation_report_name(variant, "acoustic-flow", args.backend, profile)
