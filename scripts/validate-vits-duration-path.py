#!/usr/bin/env python3
"""Require exact C++ VITS duration rounding and monotonic-path parity."""
...
def artifact_path(case: dict[str, Any], name: str) -> str:
    ...
    if input_artifact.get("name") == name and isinstance(input_artifact.get("path"), str):
...
def compare_exact(reference: Path, candidate: Path, dtype: str) -> dict[str, Any]:
    ...
    if not exact:
        mismatch = int(np.flatnonzero(expected != actual)[0])
        raise ValidationError(
            f"structural mismatch at element {mismatch}: expected {expected[mismatch]}, got {actual[mismatch]}"
        )
    return {"elements": int(expected.size), "exact": True}
...
def main() -> int:
    ...
    for case in manifest.get("cases", []):
        ...
        oracle_parameters = case.get("oracle", {}).get("parameters", {})
        noise_scale_w = oracle_parameters.get("noise_scale_w")
        speaking_rate = case.get("request", {}).get("speaking_rate")
        if not isinstance(noise_scale_w, (int, float)) or noise_scale_w < 0:
            raise ValidationError(f"{case_id}: oracle noise_scale_w must be non-negative")
        if not isinstance(speaking_rate, (int, float)) or speaking_rate <= 0:
            raise ValidationError(f"{case_id}: speaking_rate must be positive")
        reference_dir = case_root / case_id
        output_dir = output_root / case_id / "cpp" / (
            "duration_path" if args.backend == "cpu" else f"duration_path_{args.backend}"
        )
        command = with_speaker_argument([
            str(runner_path),
            str(model_path),
            str(reference_dir / artifact_path(case, "input.token_ids")),
            str(reference_dir / artifact_path(case, "random.duration_noise")),
            str(output_dir),
            str(args.threads),
            str(noise_scale_w),
            str(speaking_rate),
        ...
        probes = {}
        for logical_name, filename, dtype in (
            ("duration.w_ceil", "w_ceil.f32", "<f4"),
            ("duration.y_length", "y_length.i64", "<i8"),
            ("duration.attention", "attention.f32", "<f4"),
        ):
            probes[logical_name] = compare_exact(
    ...
    if not case_reports:
        raise ValidationError("manifest has no cases")
    report = {
    ...
    report_name = validation_report_name(variant, "duration-path", args.backend, profile)
    report_path = project_root / "reports" / "validate" / "vits" / report_name
    write_json_atomic(report_path, report)
    print(json.dumps({"status": report["status"], "cases": len(case_reports)}, sort_keys=True))
    return 0
