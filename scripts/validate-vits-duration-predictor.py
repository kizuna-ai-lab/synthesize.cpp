#!/usr/bin/env python3
"""Measure the C++ VITS stochastic duration predictor against its oracle."""
...
def main() -> int:
    ...
    for case in manifest.get("cases", []):
        ...
        noise_scale_w = parameters.get("noise_scale_w")
        ...
        output_dir = output_root / case_id / "cpp" / (
            "duration" if args.backend == "cpu" else f"duration_{args.backend}"
        )
        ...
        probe = compare_f32(
            reference_dir / artifact_path(case, "duration.logw"),
            output_dir / "logw.f32",
        )
        case_reports.append({"id": case_id, "probes": {"duration.logw": probe}})

    if not case_reports:
        raise ValidationError("manifest has no cases")
    worst_case = max(case_reports, key=lambda item: item["probes"]["duration.logw"]["max_abs"])
    worst = {
        "duration.logw": {
            "case_id": worst_case["id"],
            **worst_case["probes"]["duration.logw"],
        }
    }
    report = {
        ...
        "family": "vits",
        "variant": variant,
        "slice": "stochastic_duration_predictor",
    ...
    report_name = validation_report_name(
        variant, "duration-predictor", args.backend, profile
    )
