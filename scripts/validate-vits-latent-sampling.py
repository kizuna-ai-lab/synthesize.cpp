#!/usr/bin/env python3
"""Compare the C++ VITS latent-sampling slice with pinned PyTorch artifacts."""
...
def main() -> int:
    ...
    for case in manifest.get("cases", []):
        ...
        output_dir = output_root / case_id / "cpp" / (
            "latent_sampling" if args.backend == "cpu" else f"latent_sampling_{args.backend}"
        )
        ...
        probe = compare_f32(
            reference_dir / artifact_path(case, "latent.z_p"),
            output_dir / "z_p.f32",
        )
        case_reports.append({"id": case_id, "probes": {"latent.z_p": probe}})

    if not case_reports:
        raise ValidationError("manifest has no cases")
    worst_case = max(case_reports, key=lambda item: item["probes"]["latent.z_p"]["max_abs"])
    worst = {"latent.z_p": {"case_id": worst_case["id"], **worst_case["probes"]["latent.z_p"]}}
    report = {
        "schema": "synthesize-slice-validation-v1",
        "family": "vits",
        "variant": variant,
        "slice": "latent_sampling",
    ...
    report_name = validation_report_name(variant, "latent-sampling", args.backend, profile)
