#!/usr/bin/env python3
"""Measure the C++ VITS expanded priors against the pinned oracle."""
...
def main() -> int:
    ...
    for case in manifest.get("cases", []):
        ...
        output_dir = output_root / case_id / "cpp" / (
            "prior_expansion" if args.backend == "cpu" else f"prior_expansion_{args.backend}"
        )
        ...
        for logical_name, filename in (
            ("prior.m_p_expanded", "m_p_expanded.f32"),
            ("prior.logs_p_expanded", "logs_p_expanded.f32"),
        ):
    ...
    worst = {}
    for logical_name in ("prior.m_p_expanded", "prior.logs_p_expanded"):
    ...
    report_name = validation_report_name(variant, "prior-expansion", args.backend, profile)
