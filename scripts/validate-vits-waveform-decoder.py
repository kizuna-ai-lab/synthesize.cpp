#!/usr/bin/env python3
"""Compare the C++ VITS waveform decoder with pinned PyTorch PCM artifacts."""
...
def main() -> int:
    ...
    for case in manifest.get("cases", []):
        ...
        noise_scale = oracle_parameters.get("noise_scale")
        ...
        if not isinstance(noise_scale, (int, float)) or noise_scale < 0:
            raise ValidationError(f"{case_id}: oracle noise_scale must be non-negative")
        ...
        output_name = (
            "waveform_decoder"
            if args.backend == "cpu"
            else f"waveform_decoder_{args.backend}"
        )
        output_dir = output_root / case_id / "cpp" / output_name
        command = with_speaker_argument([
            ...
            str(reference_dir / artifact_path(case, "random.latent_noise")),
            str(output_dir),
            str(args.threads),
            str(noise_scale),
            str(noise_scale_w),
        ...
        probe = compare_f32(
            reference_dir / artifact_path(case, "audio.pcm"),
            output_dir / "pcm.f32",
        )
        case_reports.append({"id": case_id, "probes": {"audio.pcm": probe}})

    if not case_reports:
        raise ValidationError("manifest has no cases")
    worst_case = max(case_reports, key=lambda item: item["probes"]["audio.pcm"]["max_abs"])
    worst = {"audio.pcm": {"case_id": worst_case["id"], **worst_case["probes"]["audio.pcm"]}}
    report = {
        "schema": "synthesize-slice-validation-v1",
        "family": "vits",
        "variant": variant,
        "slice": "waveform_decoder",
    ...
    report_name = validation_report_name(variant, "waveform-decoder", args.backend, profile)
