...
bool load_linear(ggml_context * context, const std::string & prefix, int64_t in, int64_t out, LinearWeights & out_w) {
    out_w.weight = find(context, prefix + ".weight", { in, out });
    out_w.bias   = find(context, prefix + ".bias", { out });
    return out_w.weight != nullptr && out_w.bias != nullptr;
}

bool load_norm(ggml_context * context, const std::string & prefix, int64_t channels, NormWeights & out_w) {
    out_w.weight = find(context, prefix + ".weight", { channels });
    out_w.bias   = find(context, prefix + ".bias", { channels });
    return out_w.weight != nullptr && out_w.bias != nullptr;
}

// The text encoder's LayerNorm stores its parameters as gamma and beta.
bool load_gamma_beta(ggml_context * context, const std::string & prefix, int64_t channels, NormWeights & out_w) {
    out_w.weight = find(context, prefix + ".gamma", { channels });
    out_w.bias   = find(context, prefix + ".beta", { channels });
    return out_w.weight != nullptr && out_w.bias != nullptr;
}
...
bool load_conv(ggml_context *      context,
               ...
               int64_t             out,
               Conv1dWeights &     out_w) {
    out_w.weight = find(context, prefix + ".weight", { kernel, in, out });
    out_w.bias   = find(context, prefix + ".bias", { out });
    return out_w.weight != nullptr && out_w.bias != nullptr;
}
...
bool load_transpose_conv(ggml_context *      context,
                         ...
                         int64_t             out,
                         Conv1dWeights &     out_w) {
    out_w.weight = find(context, prefix + ".weight", { kernel, out, in });
    out_w.bias   = find(context, prefix + ".bias", { out });
    return out_w.weight != nullptr && out_w.bias != nullptr;
}
...
bool load_depthwise_transpose_conv(ggml_context *      context,
                                   ...
                                   int64_t             channels,
                                   Conv1dWeights &     out_w) {
    out_w.weight = find(context, prefix + ".weight", { kernel, 1, channels });
    out_w.bias   = find(context, prefix + ".bias", { channels });
    return out_w.weight != nullptr && out_w.bias != nullptr;
}
...
bool load_adain(ggml_context *      context,
                ...
                int64_t             channels,
                AdaINWeights &      out_w) {
    return load_linear(context, prefix + ".fc", style_dim, 2 * channels, out_w.fc);
}
...
bool load_lstm_direction(ggml_context *         context,
    ...
    out_w.weight_ih = find(context, prefix + ".weight_ih_l0" + suffix, { input_dim, 4 * hidden });
    out_w.weight_hh = find(context, prefix + ".weight_hh_l0" + suffix, { hidden, 4 * hidden });
    out_w.bias_ih   = find(context, prefix + ".bias_ih_l0" + suffix, { 4 * hidden });
    out_w.bias_hh   = find(context, prefix + ".bias_hh_l0" + suffix, { 4 * hidden });
...
bool load_lstm(ggml_context *      context,
               ...
               int64_t             hidden,
               LstmTensors &       out_w) {
    return load_lstm_direction(context, prefix, "", input_dim, hidden, out_w.forward) &&
           load_lstm_direction(context, prefix, "_reverse", input_dim, hidden, out_w.reverse);
}
...
bool load_adain_resblock(ggml_context *         context,
    ...
    if (!load_conv(context, prefix + ".conv1", 3, in, out, out_w.conv1) ||
        !load_conv(context, prefix + ".conv2", 3, out, out, out_w.conv2) ||
        !load_adain(context, prefix + ".norm1", style_dim, in, out_w.norm1) ||
        !load_adain(context, prefix + ".norm2", style_dim, out, out_w.norm2)) {
        return false;
    }
    if (in != out) {
        out_w.conv1x1 = find(context, prefix + ".conv1x1.weight", { 1, in, out });
        if (out_w.conv1x1 == nullptr) {
            return false;
        }
    }
    if (upsample && !load_depthwise_transpose_conv(context, prefix + ".pool", 3, in, out_w.pool)) {
        return false;
    }
    return true;
}
...
bool load_adain_resblock1(ggml_context *                context,
    ...
    for (size_t layer = 0; layer < layers; ++layer) {
        ...
        if (!load_conv(context, prefix + ".convs1." + index, kernel, channels, channels, out_w.convs1[layer]) ||
            !load_conv(context, prefix + ".convs2." + index, kernel, channels, channels, out_w.convs2[layer]) ||
            !load_adain(context, prefix + ".adain1." + index, style_dim, channels, out_w.adain1[layer]) ||
            !load_adain(context, prefix + ".adain2." + index, style_dim, channels, out_w.adain2[layer])) {
            return false;
        }
        // Snake alphas are stored as [1, channels, 1].
        out_w.alpha1[layer] = find(context, prefix + ".alpha1." + index, { 1, channels, 1 });
        out_w.alpha2[layer] = find(context, prefix + ".alpha2." + index, { 1, channels, 1 });
...
bool build_bert(ggml_context * context, const HParams & hparams, PLBertWeights & out_w) {
    ...
    out_w.word_embeddings     = find(context, "bert.embeddings.word_embeddings.weight", { embed_dim, hparams.n_token });
    out_w.position_embeddings = find(context, "bert.embeddings.position_embeddings.weight",
                                     { embed_dim, hparams.plbert.max_position_embeddings });
    out_w.token_type_embeddings = find(context, "bert.embeddings.token_type_embeddings.weight", { embed_dim, 2 });
    if (out_w.word_embeddings == nullptr || out_w.position_embeddings == nullptr ||
        out_w.token_type_embeddings == nullptr) {
        return false;
    }
    if (!load_norm(context, "bert.embeddings.LayerNorm", embed_dim, out_w.embedding_norm) ||
        !load_linear(context, "bert.encoder.embedding_hidden_mapping_in", embed_dim, hidden, out_w.hidden_mapping)) {
        return false;
    }
    ...
    return load_linear(context, layer + ".attention.query", hidden, hidden, out_w.layer.query) &&
           load_linear(context, layer + ".attention.key", hidden, hidden, out_w.layer.key) &&
           load_linear(context, layer + ".attention.value", hidden, hidden, out_w.layer.value) &&
           load_linear(context, layer + ".attention.dense", hidden, hidden, out_w.layer.dense) &&
           load_norm(context, layer + ".attention.LayerNorm", hidden, out_w.layer.attention_norm) &&
           load_linear(context, layer + ".ffn", hidden, hparams.plbert.intermediate_size, out_w.layer.ffn) &&
           load_linear(context, layer + ".ffn_output", hparams.plbert.intermediate_size, hidden,
                       out_w.layer.ffn_output) &&
           load_norm(context, layer + ".full_layer_layer_norm", hidden, out_w.layer.output_norm);
}
...
bool build_text_encoder(ggml_context * context, const HParams & hparams, TextEncoderWeights & out_w) {
    ...
    out_w.embedding        = find(context, "text_encoder.embedding.weight", { channels, hparams.n_token });
    ...
    for (uint32_t block = 0; block < hparams.n_layer; ++block) {
        ...
        if (!load_conv(context, prefix + ".0", kernel, channels, channels, out_w.cnn[block]) ||
            !load_gamma_beta(context, prefix + ".1", channels, out_w.cnn_norm[block])) {
            return false;
        }
    }
    return load_lstm(context, "text_encoder.lstm", channels, channels / 2, out_w.lstm);
}
...
bool build_predictor(ggml_context * context, const HParams & hparams, ProsodyPredictorWeights & out_w) {
    ...
    for (uint32_t block = 0; block < hparams.n_layer; ++block) {
        ...
        if (!load_lstm(context, lstm_prefix, lstm_in, half, out_w.text_encoder.lstms[block]) ||
            !load_linear(context, norm_prefix + ".fc", style, 2 * hidden, out_w.text_encoder.ada_norm[block])) {
            return false;
        }
    }

    if (!load_lstm(context, "predictor.lstm", lstm_in, half, out_w.lstm) ||
        !load_linear(context, "predictor.duration_proj.linear_layer", hidden, hparams.max_dur, out_w.duration_proj) ||
        !load_lstm(context, "predictor.shared", lstm_in, half, out_w.shared)) {
        return false;
    }
    ...
    for (size_t block = 0; block < 3; ++block) {
        ...
        if (!load_adain_resblock(context, "predictor.F0." + std::to_string(block), style, shape.in, shape.out,
                                 shape.upsample, out_w.f0[block]) ||
            !load_adain_resblock(context, "predictor.N." + std::to_string(block), style, shape.in, shape.out,
                                 shape.upsample, out_w.n[block])) {
            return false;
        }
    }
    return load_conv(context, "predictor.F0_proj", 1, f0_hidden, 1, out_w.f0_proj) &&
           load_conv(context, "predictor.N_proj", 1, f0_hidden, 1, out_w.n_proj);
}
...
bool build_generator(ggml_context * context, const HParams & hparams, GeneratorWeights & out_w) {
    ...
    if (!load_linear(context, "decoder.generator.m_source.l_linear", hparams.source.harmonic_num + 1, 1,
                     out_w.source_linear)) {
        return false;
    }
    ...
    for (size_t stage = 0; stage < stages; ++stage) {
        ...
        if (!load_transpose_conv(context, "decoder.generator.ups." + std::to_string(stage),
        ...
        for (size_t branch = 0; branch < net.resblock_kernel_sizes.size(); ++branch) {
            ...
            if (!load_adain_resblock1(context, "decoder.generator.resblocks." + std::to_string(index), style, next,
                                      net.resblock_kernel_sizes[branch], net.resblock_dilations[branch],
        ...
        if (!load_conv(context, "decoder.generator.noise_convs." + std::to_string(stage), noise_kernel, bins, next,
                       out_w.noise_convs[stage])) {
            return false;
        }
        ...
        if (!load_adain_resblock1(context, "decoder.generator.noise_res." + std::to_string(stage), style, next,
                                  noise_res_kernel, noise_dilations, out_w.noise_res[stage])) {
            return false;
        }
        channels = next;
    }
    return load_conv(context, "decoder.generator.conv_post", 7, channels, bins, out_w.conv_post);
}
...
bool build_decoder(ggml_context * context, const HParams & hparams, DecoderWeights & out_w) {
    ...
    if (!load_conv(context, "decoder.F0_conv", 3, 1, 1, out_w.f0_conv) ||
        !load_conv(context, "decoder.N_conv", 3, 1, 1, out_w.n_conv) ||
        !load_conv(context, "decoder.asr_res.0", 1, hidden, kAsrResChannels, out_w.asr_res)) {
        return false;
    }
    if (!load_adain_resblock(context, "decoder.encode", style, hidden + kCurveChannels, kDecoderEncodeOut, false,
                             out_w.encode)) {
        return false;
    }
    ...
    for (size_t block = 0; block < block_count; ++block) {
        ...
        if (!load_adain_resblock(context, "decoder.decode." + std::to_string(block), style, decode_in, out, last,
                                 out_w.decode[block])) {
            return false;
        }
    }
...
bool build_voices(ggml_context * context, const HParams & hparams, VoiceWeights & out_w) {
    ...
    for (const std::string & id : hparams.preset_voice_ids) {
        ggml_tensor * pack = find(context, "voice." + id, { hparams.voice_dim, hparams.voice_rows });
        if (pack == nullptr) {
            return false;
        }
