...
synth_device_flags_t device_memory_flags(BackendKind                  kind,
    ...
    if (device_type == GGML_BACKEND_DEVICE_TYPE_IGPU || kind == BackendKind::Metal ||
        kind == BackendKind::Accel) {
...
synth_status_t get_backend_device(ggml_backend_dev_t       device,
    ...
    staged.memory_total = (flags & SYNTH_DEVICE_MEMORY_INFO_VALID) == 0
                              ? 0
                              : static_cast<uint64_t>(properties.memory_total);
    staged.memory_free = (flags & SYNTH_DEVICE_MEMORY_INFO_VALID) == 0
                             ? 0
                             : static_cast<uint64_t>(properties.memory_free);
...
synth_status_t resolve_cpu_device(synth_backend_request_t request,
    ...
    if (request != SYNTH_BACKEND_AUTO && request != SYNTH_BACKEND_CPU &&
        request != SYNTH_BACKEND_CPU_ACCEL) {
        if (output != nullptr) {
            *output = nullptr;
        }
