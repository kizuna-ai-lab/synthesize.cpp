...
int main() {
    ...
    SYNTH_TEST_CHECK(prepared.noise_channel_fastest ==
    ...
    SYNTH_TEST_CHECK(synth::vits::prepare_duration_input(
                         3, source, std::numeric_limits<float>::quiet_NaN(), prepared) == SYNTH_ERR_INVALID_ARG);
    ...
    SYNTH_TEST_CHECK(synth::vits::finalize_duration_output(
                         1, { std::numeric_limits<float>::infinity() }, output) == SYNTH_ERR_INTERNAL);
