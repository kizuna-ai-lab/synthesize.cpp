...
int main() {
    ...
    SYNTH_TEST_CHECK(synth::vits::resolve_duration_path(
                         hparams, { 0.0f }, std::numeric_limits<float>::quiet_NaN(), output) == SYNTH_ERR_INVALID_ARG);
    SYNTH_TEST_CHECK(synth::vits::resolve_duration_path(
                         hparams, { std::numeric_limits<float>::infinity() }, 1.0f, output) ==
                     SYNTH_ERR_INVALID_ARG);
    SYNTH_TEST_CHECK(synth::vits::resolve_duration_path(hparams, { 10.0f }, 1.0f, output) ==
                     SYNTH_ERR_OUTPUT_LIMIT);
    SYNTH_TEST_CHECK(output.w_ceil.empty() && output.attention.empty());
    SYNTH_TEST_CHECK(synth::vits::resolve_duration_path(
                         hparams, { std::log(20.0f), std::log(10.0f) }, 1.0f, output) == SYNTH_ERR_OUTPUT_LIMIT);
