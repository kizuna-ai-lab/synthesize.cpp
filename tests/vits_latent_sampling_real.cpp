...
int main(int argc, char ** argv) {
    if (argc != 10 && argc != 11 && argc != 12) {
        std::cerr << "usage: synthesize-vits-latent-sampling MODEL.gguf token_ids.i32 duration_noise.f32 "
    ...
    if (!read_values(argv[2], token_ids) || !read_values(argv[3], duration_noise) ||
        ...
        std::cerr << "failed to read latent-sampling inputs\n";
        return 3;
    }
    ...
    synth::vits::LatentSamplingOutput result;
    status = model->run_latent_sampling(token_ids, duration_noise, latent_noise, noise_scale, noise_scale_w,
                                        speaking_rate, threads, result, speaker_index);
    if (status != SYNTH_OK) {
        std::cerr << "latent sampling failed: " << synth_status_string(status) << '\n';
        return 5;
    }
    ...
    if (error || !write_values(output_dir / "z_p.f32", result.z_p)) {
        std::cerr << "failed to write latent-sampling output\n";
        return 6;
    }
