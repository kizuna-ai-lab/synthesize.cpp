...
int main() {
    ...
    SYNTH_TEST_CHECK(prepared.z_channel_fastest ==
    ...
    SYNTH_TEST_CHECK(synth::vits::finalize_waveform_decoder_output(
                         1, { std::numeric_limits<float>::quiet_NaN() }, output) == SYNTH_ERR_INTERNAL);
