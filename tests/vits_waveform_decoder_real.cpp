...
int main(int argc, char ** argv) {
    if (argc != 10 && argc != 11 && argc != 12) {
        std::cerr << "usage: synthesize-vits-waveform-decoder MODEL.gguf token_ids.i32 duration_noise.f32 "
    ...
    if (!read_values(argv[2], token_ids) || !read_values(argv[3], duration_noise) ||
        ...
        std::cerr << "failed to read waveform-decoder inputs\n";
        return 3;
    }
    ...
    synth::vits::WaveformDecoderOutput result;
    status = model->run_waveform_decoder(token_ids, duration_noise, latent_noise, noise_scale, noise_scale_w,
                                         speaking_rate, threads, result, speaker_index);
    if (status != SYNTH_OK) {
        std::cerr << "waveform decoder failed: " << synth_status_string(status) << '\n';
        return 5;
    }
    ...
    if (error || !write_values(output_dir / "pcm.f32", result.pcm)) {
        std::cerr << "failed to write waveform-decoder output\n";
        return 6;
    }
